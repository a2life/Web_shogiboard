...
ＣＳＳファイルと将棋のイメージファイルのパスはboarddata.jsにコーディングしてあるので、
これらのファイルの位置を動かす場合は修正して使用してください。


本流ではないので、とりあえず動くもの、と考えてください。PHP/MODX solution とは同じJavaScript Engine　で動いてはおりますが、
細かい表示パラメーターを生のJSデータで入れ込むのはプログラムがわかっていないと無理です。(PHP/MODXでは、簡単なtag 入力）
また、JavaScriptのkifパーサは作るつもりはありません。
このフォルダーのboarddata.js に入れてある棋譜はPHP/MODXソリューションのパーサーで生成したものです。
これを手で入力するのは大変なので、実際の使い方としては簡単な詰将棋とか、盤面の表示くらいに限定されると思います。


一応分岐にも対応してます、盤面配置の調整などはCSS とboardsetup.js と ＣＳＳのチューニングだけでできますのでご自由にどうぞ。
