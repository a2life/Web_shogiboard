...
This was primarily conceived to be used on my shogi learning site to show various moves and explain shogi plays. See shogishack.net for actual usage.

The program works on MODX Revolution CMS.  the coding is in PHP and javascript.  PHP utilizes several MODX APIs and utilities available through its manager screen. 

(JavaScriptOnlySolutin contains demo.html  and other helper js files to render this board without a need of modx nor PHP. Current solution is fairly comparable with modx/PHP solution however, it lacks kifufile parser. I have no intention of creating kifu parser with Javascript. This means you need to hand code moves as illustrated below. Demo uses kifu moves that were parsed abd generated by PHP code.)
